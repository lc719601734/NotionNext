...
export function getMatchingLanguagePrefix(userLang, pageId) {
  const normalizedUserLang = userLang?.toLowerCase().split(/[-_]/)[0]
  if (!normalizedUserLang) {
    return ''
  }
  ...
  return (
    siteIds
      .map(siteId => extractLangPrefix(siteId).toLowerCase())
      .find(prefix => prefix === normalizedUserLang) || ''
  )
}
...
export const redirectUserLang = (lang, pageId) => {
  ...
  const matchedPrefix = getMatchingLanguagePrefix(userLang, pageId)

  // 默认是进首页; 如果检测到有一个多语言匹配了用户浏览器，则自动跳转过去
  if (matchedPrefix && window.location.pathname === '/') {
    window.location.href = '/' + matchedPrefix
  }
}